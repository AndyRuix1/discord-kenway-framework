...
> * [Crear instancia de  un nuevo cliente](#🛠-instanciar-un-nuevo-bot)
...
* <small>(*)</small> - ID - [<small>Obtener</small>](https://discord.com/developers/applications)
...
const pongTypeOption = new SlashCommandStringOption()
	...
	.addChoices({ name: 'grande', value: 'big' });
...
export  default  new  Command({ 
	...
	execute: (client, interaction) => {
		...
		if (customSize) return interaction.reply({ content: `https://picsum.photos/${customSize.split('x')[0]}/${customSize.split('x')[1]}`, ephemeral: true });
		interaction.reply({ content:`https://picsum.photos/${subCommand.split('x')[0]}/${subCommand.split('x')[1]}`, ephemeral: true });
	}
})
...
export default new Event({
	...
	execute:  async (client, message) => {
		...
		if (message.content.includes('!hola')) message.channel.send({ content: 'Hola!' });
	}
})
...
#### Ejemplo de una estructura de archivos:
>##### Recuerda que puedes usar archivos .ts
```
├── comandos
│   ├── admin
│   │   ├── ban.js
...
<div style="display: flex; justify-content: center; width:100%">
    <img style="border-radius: 23px; width: 50%" src=""/>
</div>
<div style="width:100%;">
